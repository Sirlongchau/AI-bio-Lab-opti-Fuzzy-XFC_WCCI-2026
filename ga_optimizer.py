...
ga_optimizer.py — v2
====================
GA pour le contrôleur multimodal Kessler, aligné sur targeting_system.py v2.

Changements vs v1:
  - repair() systématique (init, random, from_dict) → plus de génomes invalides exportés
  - Schéma de params aligné sur le nouveau targeting (HIT_FRACTION, TRACK_TOL, ...)
  - Mutation sigma proportionnelle à la plage de chaque paramètre
  - Pool persistant (1 seul spawn pour tout le run)
  - Tâches au grain (génome, scénario) → load balancing optimal
  - Cache du solveur CasADi par worker (économise ~2-5 s / scénario)
  - Déduplication des génomes par hash
  - Screening optionnel : évaluation courte pour tous, complète pour le top
"""
...
# ---------------------------------------------------------------------------
# Parameter schema — aligné sur targeting_system.py v2
# ---------------------------------------------------------------------------
...
PARAM_SPECS: List[ParamSpec] = [
    # ── Supervisor ────────────────────────────────────────────────────────
    ParamSpec("R_lo", 0.50, 0.05, 0.90, "supervisor"),
    ParamSpec("R_hi", 0.65, 0.15, 0.98, "supervisor"),
    ...
    ParamSpec("out_high",       0.75, 0.30, 0.99, "risk_field"),
    ParamSpec("out_medium",     0.20, 0.10, 0.80, "risk_field"),
    ParamSpec("out_low",        0.15, 0.02, 0.50, "risk_field"),
    ParamSpec("out_negligible", 0.02, 0.00, 0.20, "risk_field"),

    # ── Targeting v2 ──────────────────────────────────────────────────────
    # score = risk * (1 + W_SMALL * size_bonus)  → multiplicatif, plage 0-1
    ParamSpec("W_SMALL",         0.30, 0.0,    1.0, "targeting"),
    ParamSpec("MAX_DIST_FIRE", 700.0, 200.0, 1200.0, "targeting"),
    ParamSpec("HIT_FRACTION",    0.80, 0.40,   1.0, "targeting"),
    ParamSpec("SWITCH_RATIO",    1.30, 1.0,    3.0, "targeting"),
    ParamSpec("LOCK_MIN_FRAMES", 4.0,  1.0,   15.0, "targeting", is_int=True),
    ParamSpec("TRACK_TOL",      30.0, 10.0,   80.0, "targeting"),

    # ── Sacrifice ─────────────────────────────────────────────────────────
    ParamSpec("MINE_LEAD_S",      0.20, 0.0, 3.0, "targeting"),
    ParamSpec("MINE_MIN_THREATS", 1.0,  1.0, 5.0, "targeting", is_int=True),
]
...
class Genome:
    ...
    def repair(self) -> None:
        # Clip global dans les bornes du schéma
        ...
        # Supervisor : R_lo < R_hi (hysteresis correcte)
        ordpair("R_lo", "R_hi", 0.05)

        # Tau monotone
        ordpair("tau_crit_b",  "tau_crit_d",  0.05)
        ...
        # Distance monotone
        ordpair("d_near_c", "d_near_d", 5.0)
        ordpair("d_med_a",  "d_med_d", 10.0)

        # Singletons ordonnés : negligible < low < medium < high
        ordpair("out_negligible", "out_low",    0.01)
        ordpair("out_low",        "out_medium", 0.01)
        ordpair("out_medium",     "out_high",   0.01)

        # Re-clip après les ordpair (peuvent sortir des bornes)
        np.clip(self.genes, _LO, _HI, out=self.genes)
...
def apply_genome_to_modules(d: Dict[str, float]) -> None:
    ...
    try:
        ...
        ts.LOCK_MIN_FRAMES  = int(d["LOCK_MIN_FRAMES"])
        ts.TRACK_TOL        = d["TRACK_TOL"]
        ts.MINE_LEAD_S      = d["MINE_LEAD_S"]
        ts.MINE_MIN_THREATS = int(d["MINE_MIN_THREATS"])
    except ImportError:
        pass


# ---------------------------------------------------------------------------
# Worker persistant — état global par process
# ---------------------------------------------------------------------------

_W: dict = {}    # cache worker : modules importés + solveurs CasADi par map_size


def _worker_init() -> None:
    """Initializer du pool : payé UNE fois par worker pour tout le run."""
    global _W
    from kesslergame import Scenario, KesslerGame, GraphicsType
    from Fuzzy_MPC_Controller import Controller
    _W = {
        "Scenario":    Scenario,
        "KesslerGame": KesslerGame,
        "GraphicsType": GraphicsType,
        "Controller":  Controller,
        "solvers":     {},          # map_size → _CasADiSolver pré-construit
    }


def _make_controller(map_size: Tuple[float, float]):
    """
    Crée un Controller frais en réutilisant le solveur CasADi du worker.
    Le NLP ne dépend ni du génome ni du scénario → cache valide.
    """
    ctrl = _W["Controller"]()
    try:
        mpc = ctrl.supervisor.controllers["mpc"]
        key = tuple(map_size)
        mpc._ensure_solver(key)
        cached = _W["solvers"].get(key)
        if cached is not None:
            cached._u_prev = None           # état propre
            mpc._casadi_slv = cached
        elif mpc._casadi_slv is not None:
            import Casadi_mpc as cm
            mpc._casadi_slv._build(cm.LAMBDA_D)   # build une fois...
            _W["solvers"][key] = mpc._casadi_slv  # ...cache pour les suivants
    except Exception:
        pass                                # dégradé gracieux : rebuild lazy
    return ctrl


def _eval_task(args) -> Tuple[int, dict]:
    """
    UNE tâche = UN génome sur UN scénario.
    Grain fin → load balancing optimal sur le pool.
    """
    gid, genes, scenario_cfg, settings_override = args
    genome = Genome(genes)                  # repair() automatique
    apply_genome_to_modules(genome.to_dict())
...
def compute_fitness(scenario_results: List[dict]) -> float:
    """Fitness moyenne par scénario, avec gate de survie."""
...
def gaussian_mutate(genome, sigma_frac, p_mutate, rng) -> Genome:
    ...
    # σ proportionnel à la PLAGE de chaque paramètre — fix du bug _GROUP_SIGMA
    noise = rng.normal(0.0, sigma_frac * _RANGE)
...
class GAConfig:
    ...
    time_limit:      float = 40.0        # s de jeu par scénario (60 → 40 = -33%)
    # Screening : tous les génomes sur les scénarios courts,
    # seuls les top screen_frac passent les scénarios complets
    use_screening:   bool  = True
    screen_frac:     float = 0.35
    scenario_configs:       List[dict] = field(default_factory=list)
    screen_scenario_configs: List[dict] = field(default_factory=list)
...
class GeneticOptimizer:
    ...
    def __init__(self, config: GAConfig) -> None:
        ...
        self._cache: Dict[bytes, float] = {}     # dédup : génome → fitness
        self._pool = None
        self.history: List[dict] = []

        if not self.cfg.scenario_configs:
            full, screen = self._build_fixed_scenarios()
            self.cfg.scenario_configs        = full
            self.cfg.screen_scenario_configs = screen

    # ------------------------------------------------------------------

    def _build_fixed_scenarios(self) -> Tuple[List[dict], List[dict]]:
        rng = np.random.default_rng(self.cfg.seed)
        full, screen = [], []
        specs = [("Sparse", 10), ("Medium", 20), ("Dense", 25), ("Stress", 35)]
        for name, n_ast in specs:
            ...
            full.append({**base, "name": f"GA_{name}",
                         "time_limit": self.cfg.time_limit})
        # Screening : 2 scénarios représentatifs, durée réduite de moitié
        for s in (full[1], full[3]):                 # Medium + Stress
            screen.append({**s, "name": s["name"] + "_screen",
                           "time_limit": self.cfg.time_limit / 2})
        return full, screen
    ...
    def _ensure_pool(self):
        if self._pool is None:
            ...
            self._pool = ctx.Pool(processes=self.cfg.n_workers,
                                  initializer=_worker_init)
        return self._pool

    def _run_tasks(self, genomes: List[Tuple[int, Genome]],
                   scenarios: List[dict]) -> Dict[int, List[dict]]:
        """Évalue chaque (génome, scénario) comme tâche indépendante."""
        tasks = [(gid, g.genes.copy(), cfg, self.cfg.game_settings_override)
                 for gid, g in genomes for cfg in scenarios]
        results: Dict[int, List[dict]] = {gid: [] for gid, _ in genomes}

        if self.cfg.n_workers > 1:
            pool = self._ensure_pool()
            for gid, r in pool.imap_unordered(_eval_task, tasks, chunksize=1):
    ...
    def _evaluate(self, population: List[Genome]) -> List[float]:
        """Dédup + screening optionnel + évaluation complète."""
        fits: List[Optional[float]] = [None] * len(population)

        # 1. Cache (élites, doublons de crossover, génomes revisités)
        ...
        # 2. Screening — élimine les mauvais sur scénarios courts
        if self.cfg.use_screening and len(fresh) > 4:
            screen_res = self._run_tasks(fresh, self.cfg.screen_scenario_configs)
            screen_fit = {gid: compute_fitness(rs) for gid, rs in screen_res.items()}
            order = sorted(fresh, key=lambda t: screen_fit[t[0]], reverse=True)
            n_full = max(2, int(math.ceil(len(fresh) * self.cfg.screen_frac)))
            promoted, rest = order[:n_full], order[n_full:]
            for gid, g in rest:
        ...
        # 3. Évaluation complète des promus
        full_res = self._run_tasks(promoted, self.cfg.scenario_configs)
        for gid, g in promoted:
            f = compute_fitness(full_res[gid])
            fits[gid] = f
            self._cache[g.key()] = f
    ...
    def run(self) -> Genome:
        cfg = self.cfg
        print(f"\n{'='*64}")
        print(f"  Kessler GA v2 — {N_PARAMS} params | pop {cfg.pop_size} "
              f"| gen {cfg.n_generations} | workers {cfg.n_workers}")
    ...
    def export_best(self, path: str = "best_params.json") -> None:
        ...
        self._best_genome.repair()           # ceinture + bretelles
