...
from ga_optimizer import _patch_targeting_controller_init
...
if Path(BEST_PARAMS_PATH).exists():
    genome = GeneticOptimizer.load_and_apply(BEST_PARAMS_PATH)
    with open(BEST_PARAMS_PATH) as f:
        _d = json.load(f)
    print(f"✓ Best params loaded ({len(_d)} parameters)")
    print(f"  Supervisor  R_lo={_d.get('R_lo', 0.80):.3f}  R_hi={_d.get('R_hi', 0.90):.3f}")
    print(f"  Targeting   Kp={_d.get('Kp', 5.0):.2f}  Kd={_d.get('Kd', 0.3):.2f}")
    print(f"              FIRE_CONE={_d.get('FIRE_CONE_DEG', 4.0):.1f}°  "
